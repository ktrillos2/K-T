...
export default function RelatedPosts({ currentSlug, category, limit = 3 }: RelatedPostsProps) {
  ...
  const differentCategoryPosts = category ? otherPosts.filter((p) => p.category !== category) : otherPosts

  const selectedPosts: BlogPostItem[] = [...sameCategoryPosts, ...differentCategoryPosts].slice(0, limit)

  if (selectedPosts.length === 0) return null

  return (
    <section aria-label="Artículos relacionados" className="not-prose my-16 border-t border-white/10 pt-12">
